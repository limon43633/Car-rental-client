            ...
            {bookings.map((booking, index)=>(
              <tr key={index} className='border-t border-border-color text-gray-500'>
                <td className='p-3 flex items-center gap-3'>
                  <img src={booking.car.image} alt="" className='h-15 w-15 aspect-square rounded-md object-cover'/>
                ...
                <td className='p-3'>
                  {booking.status === 'pending' ? (
                    <select onChange={e=> changeBookingStatus(booking._id, e.target.value)} value={booking.status} className='px-2 py-1.5 mt-1 text-gray-500 border border-border-color rounded-md outline-none'>
